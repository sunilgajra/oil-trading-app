...
var getDensity=function(p){return Number(state.densities[p] || DEF_D[p] || 0.850);};
var toKG=function(v,d){return Number(v||0)*(Number(d||0.85));};
...
async function seedDefaultsIfEmpty() {
  ...
  if (!existingProducts || existingProducts.length === 0) {
    await supabase.from('products').insert(
      DEF_P.map(function(name){ return { name:name, density:DEF_D[name] || 0.850 }; })
    );
  }
}
...
async function loadState() {
  ...
  (productsRes.data || []).forEach(function(p){
    state.densities[p.name] = Number(p.density || 0.850);
  });
...
function exportInventoryExcel() {
  try {
    ...
    for (var r = 0; r < rows.length; r++) {
      ...
      html += '<td class="num">' + Number(rows[r][6]).toFixed(2) + '</td>';
...
function renderDashboardKpis() {
  ...
  for (var i = 0; i < state.inventory.length; i++) {
    ...
    tv += Number(state.inventory[i].vol);
  }
  var sl = 0;
  for (var i = 0; i < state.trades.length; i++) {
    if (state.trades[i].type === 'Sell') sl += Number(state.trades[i].vol) * Number(state.trades[i].price);
  }
...
function renderInvLevels() {
  document.getElementById('invLevels').innerHTML = state.inventory.map(function(i) {
    var p = Math.min(100, Math.round(Number(i.vol) / (Number(i.threshold) * 10) * 100));
...
function renderInventoryTable() {
  ...
  document.getElementById('invTable').innerHTML = state.inventory.filter(function(i){
    return i.product.toLowerCase().indexOf(q) >= 0;
  }).map(function(i) {
    var lvPct = Math.min(100, Number(i.vol) / Number(i.threshold) * 10);
    return '<tr><td><b>'+i.product+'</b></td><td>'+i.grade+'</td><td class="mono">'+i.density+'</td><td>'+i.tank+'</td><td class="mono">'+fmtN(i.vol)+'</td><td class="mono">'+fmtKG(toKG(i.vol,i.density))+'</td><td class="mono">'+fmt(i.cost)+'</td><td class="mono">'+fmt(Number(i.vol)*Number(i.cost))+'</td><td><div class="progress" style="width:60px"><div class="progress-fill '+(Number(i.vol)>Number(i.threshold)?'green':'red')+'" style="width:'+lvPct+'%"></div></div></td><td><button class="btn btn-danger btn-sm" onclick="deleteItem(\'inventory\','+i.id+')">✕</button></td></tr>';
  }).join('');
}
async function addInventory() {
  ...
  renderInventoryTable(); renderDashboardKpis(); renderInvLevels(); clearInvForm();
...
async function addTrade() {
  ...
  renderTradesTable(); renderRecentTrades(); renderDashboardKpis();
...
async function addOrder() {
  ...
  renderOrdersTable(); renderActiveOrders();
  toast('Created ' + id);
}
async function updateOrderStatus(id, s) {
  ...
  renderOrdersTable(); renderActiveOrders(); toast('Status updated');
}
...
function renderReports() {
  ...
  for (var i = 0; i < state.trades.length; i++) {
    var t = state.trades[i];
    if (t.type === 'Sell') cust[t.party] = (cust[t.party]||0) + (Number(t.vol) * Number(t.price));
  }
...
async function deleteItem(arr, id) {
  const tableMap = { inventory:'inventory', trades:'trades', suppliers:'suppliers' };
...
async function deleteOrder(id) {
  customConfirm('Delete order ' + id + '?').then(async function(ok) {
    ...
    renderOrdersTable(); renderActiveOrders(); toast('Order removed');
  });
}
async function deleteChallan(id) {
  customConfirm('Delete challan ' + id + '?').then(async function(ok) {
    ...
    renderChallansTable(); toast('Challan removed');
  });
}
async function deleteProduct(n) {
  customConfirm('Delete product "' + n + '"?').then(async function(ok) {
    ...
    populateSelects(); renderProductsList(); toast('Product removed');
  });
}
async function addProductMaster() {
  ...
  var row = {
    name: n,
    density: parseFloat(document.getElementById('pm-density').value) || 0.85
  };
  ...
  state.densities[data.name] = Number(data.density || 0.85);
  populateSelects(); renderProductsList();
